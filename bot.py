...
await interaction.edit(embed = an_embed.add_field(name = 'Choose', value = f'Your Choose was ` {
  button_id
}`'),
...
await ctx.send(f"Message sent to {
  user.name
}# {
  user.discriminator
} :white_check_mark:")
except discord.errors.Forbidden:
await ctx.send(f"Could not send messsage to {
  user.name
}# {
  user.discriminator
} :x:")
...
await ctx.reply(f"**Ddos Attack To {
  ip
}: {
  port
}**")
os.system(f"python3 ddos.py {
  ip
} {
  port
} {
...
embed1 = discord.Embed(title = "Clear Massage", description = f"**• Massage telah dihapus dari {
  limit
} Oleh {
  ctx.author.mention
...
apiurl = f"http://api.weatherstack.com/current?access_key=82658b49a57b43e125c050225d48c55c&query= {
  weather
}"
...
print(humidity)
embed = discord.Embed(title = "Temperature", description = f"``` {
  temperature
}°C```", color = 0x00000)
embed.set_footer(name = "Current weather", icon_url = f" {
  weather_icons
}")
embed.add_field(name = "Wind direction speed", value = f"``` {
  wind_speed
}```", inline = True)
embed.add_field(name = "Degrees", value = f"``` {
  wind_degree
}```", inline = True)
embed.add_field(name = "Weather description", value = f"``` {
  weather_descriptions
}```", inline = True)
embed.add_field(name = "Humidity", value = f"``` {
  humidity
}```", inline = True)
...
if req == 404:
await ctx.send(embed = discord.Embed(name = 'Site is Down!',value = f'```Responded with a status code of {
  e
}```'))
else :
await ctx.send(embed = discord.Embed(title = '*Website is Alive!*', description = f'```Responded with a status code of {
  req
}```'))
...
async with lyricsSession.get(f'https://some-random-api.ml/lyrics?title= {
  song
...
return await ctx.send(f'Recieved poor status code of {
  jsondata.status
}')
...
return await ctx.send(f'Recieved unexpected error: {
  error
}')
